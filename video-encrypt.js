...
const outputDir = path.join(__dirname, "enc_videos");
...
function encryptBinary(inputPath, outputPath) {
  ...
  debugLog("🔒 Encrypted:", path.relative(__dirname, inputPath));
}
...
function encryptAndRewriteM3U8(inputPath, outputPath) {
  ...
  debugLog("📄 Encrypted playlist:", path.relative(__dirname, inputPath));
}

function copyThumbnail(src, dest) {
  fs.copyFileSync(src, dest);
  debugLog("🖼️ Copied thumbnail:", path.relative(__dirname, src));
}
